...
/*3.1.2. Вывести студентов, которые сдавали дисциплину «Основы баз данных», указать дату попытки и результат. 
...
from
student JOIN attempt USING (student_id) JOIN subject USING (subject_id)
WHERE subject_id = 2
ORDER BY result DESC;


/*3.1.3. Вывести, сколько попыток сделали студенты по каждой дисциплине, а также средний результат попыток, который округлить до 2 знаков после запятой. 
...
SELECT name_subject, count(attempt.subject_id) as Количество, ROUND(AVG(result),2) as Среднее FROM
subject LEFT JOIN attempt USING (subject_id)
GROUP BY name_subject
ORDER BY Среднее DESC;

/*3.1.4. Вывести студентов (различных студентов), имеющих максимальные результаты попыток. 
...
SELECT name_student, result FROM 
attempt JOIN student USING (student_id)
WHERE result = (SELECT max(result) from attempt)
ORDER by name_student;

/*3.1.5. Если студент совершал несколько попыток по одной и той же дисциплине, то вывести разницу в днях между первой и последней попыткой. В результат включить фамилию и имя студента, название дисциплины и вычисляемый столбец Интервал. 
Информацию вывести по возрастанию разницы. 
Студентов, сделавших одну попытку по дисциплине, не учитывать.
*/

SELECT name_student, name_subject, DATEDIFF(max(date_attempt), min(date_attempt)) as Интервал FROM
student JOIN attempt USING (student_id)
JOIN subject USING (subject_id)
GROUP BY name_student, name_subject HAVING COUNT(date_attempt)>1
ORDER BY Интервал;

/*3.1.6. Студенты могут тестироваться по одной или нескольким дисциплинам (не обязательно по всем). 
...
SELECT name_subject, count(DISTINCT(student_id)) as Количество FROM
subject LEFT JOIN attempt USING (subject_id)
GROUP by name_subject
ORDER BY Количество DESC, name_subject;

/*3.1.7. Случайным образом отберите 3 вопроса по дисциплине «Основы баз данных». 
...
/*3.1.8. Вывести вопросы, которые были включены в тест для Семенова Ивана по дисциплине «Основы SQL» 2020-05-17  (значение attempt_id для этой попытки равно 7). 
...
SELECT name_question, name_answer, IF(is_correct, 'Верно', 'Неверно') AS Результат FROM
answer JOIN testing USING (answer_id)
JOIN question ON testing.question_id = question.question_id
WHERE attempt_id = 7;

/*3.1.9. Посчитать результаты тестирования. 
...
SELECT name_subject, 
       ...
       ROUND(SUM(is_correct) / COUNT(answer_id) * 100, 2) AS Успешность FROM
      attempt JOIN testing USING (attempt_id)
JOIN answer USING (answer_id)
JOIN question  ON answer.question_id = question.question_id
JOIN subject ON subject.subject_id = question.subject_id
